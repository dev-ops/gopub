...
<template>
  <div class="login-bodya">
    <div class="loginWarpa">
      ...
      <div v-if="get_user_info.login" class="login-forma">
        <el-form ref="form" :model="form" :rules="rules" label-width="0">
          <el-form-item prop="old_password" class="login-itema">
            <label>用户名 ：</label>
            <span v-text="get_user_info.user.Username"/>
          </el-form-item>

          <el-form-item prop="old_password" class="login-itema">
            <label>邮箱 ：</label>
            <span v-text="get_user_info.user.Email"/>
          </el-form-item>
...
export default {
  ...
  methods: {
    ...
    submit_form() {
      this.$http
        .post(portUser.changepasswd, this.form)
        .then(({ data: { msg } }) => {
          this.$message({
            message: msg,
            type: "success"
